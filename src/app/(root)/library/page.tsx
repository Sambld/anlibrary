  ...
  return (
    <>
     <div className="text-2xl font-bold p-10 max-sm:p-4 ">
      <p className="text-2xl font-bold">My Library <span className="text-sm">({library.length})</span></p>

      {airingToday.length > 0 && (
        <div className="mt-5">
          <div className="border-l-teal-600 border-l-8 pl-4 py-2 ">
            <h2 className="text-xl ">Airing Today <span className=" text-sm"> ({airingToday.length})</span></h2> 
          </div>
    ...
    <div className="flex  flex-col mt-10 gap-2 justify-center items-center text-center">
      {library.length === 0 && ( 
        <div className="mt-5">
          ...
          <Link href="/">
            <Button  className="mt-5 border" variant="secondary">Discover</Button>
          </Link>
        </div>
      )}
    </div>
